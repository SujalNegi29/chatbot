...
class ActionAddToCart(Action):
    ...
    def run(
        ...
        if not food:
            dispatcher.utter_message("❌ Please tell me which food item you want.")
            return []
        ...
        if not result:
            dispatcher.utter_message(f"❌ {food} is not available in our menu.")
            conn.close()
            return []
        ...
        dispatcher.utter_message(
            f"✅ Added {quantity} {food}(s) to your cart (₹{total_price})"
        )
...
class ActionShowTotal(Action):
    ...
    def run(
        ...
        for food, qty, price in rows:
            total += price
            lines.append(f"{food} x {qty} = ₹{price}")
...
class ActionReorderLast(Action):
    ...
    def run(
        ...
        if not row:
            dispatcher.utter_message("⚠️ You don’t have any previous orders.")
            conn.close()
            return []
        ...
        dispatcher.utter_message(
            "🔁 Items from your last order have been added to your cart."
        )
